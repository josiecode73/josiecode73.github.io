...
$(function () {
  ...
  $grid.find('.thumb-video').each(function () {
    ...
    if (playPromise !== undefined) {
      playPromise.then(function() {
        // Wait a tiny bit before pausing to avoid the interruption error
        setTimeout(function() {
            try {
                v.pause();           // 即停止
  ...
  $grid.on('mouseleave', '.is-video video', function () {
    var v = this;
    // Wait a tiny bit to ensure play() has resolved before pausing to prevent interruption error
    setTimeout(function() {
      try {
        v.pause();
      } catch(e) {}
